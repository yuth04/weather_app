...
const WeatherDashboard = () => {
  ...
  const fetchDailyAndUv = useCallback(async (lat, lon) => {
    try {
      ...
      if (!res.ok || data.error) throw new Error(data.reason || "Failed to fetch 7-day forecast.");
      return data;
    } catch (err) {
  ...
  const fetchForecast = useCallback(
    async (cityName, currentWeather) => {
      try {
        ...
        const dailyAndUvData = await fetchDailyAndUv(currentWeather.coord.lat, currentWeather.coord.lon);
        const currentUvIndex = dailyAndUvData?.hourly?.uv_index?.[0] || "N/A";

        const todayForecast = dailyAndUvData.daily.time.map((t, i) => ({
        ...
        // Ensure today's min/max includes current temp
  ...
  useEffect(() => {
    if (!weatherData.daily || !weatherData.main) return;
    const updatedDaily = weatherData.daily.map((d, i) => {
      if (i === 0) {
        ...
        return {
          ...
          main: {
            ...d.main,
            temp_max: Math.max(d.main.temp_max, currentTemp),
            temp_min: Math.min(d.main.temp_min, currentTemp),
          },
        };
      }
      return d;
    });
    setWeatherData((prev) => ({ ...prev, daily: updatedDaily }));
    ...
    <div className="flex flex-col md:flex-row min-h-screen bg-gradient-to-br from-gray-900 to-gray-800 text-white p-4 md:p-6">
      ...
      <aside className="flex md:flex-col justify-center md:justify-start md:w-20 items-center space-x-6 md:space-x-0 md:space-y-6 py-4 md:py-8 border-b md:border-b-0 md:border-r border-gray-700">
        ...
        <nav className="flex md:flex-col space-x-6 md:space-x-0 md:space-y-6 text-gray-400">
          {[<CloudSun size={28} />, <Building size={28} />, <Map size={28} />, <Settings size={28} />].map(
            (icon, idx) => (
              <button key={idx} className="hover:text-white transition-colors duration-200">
                {icon}
              </button>
            )
          )}
      ...
      <main className="flex-1 px-2 md:px-8 mt-4 md:mt-0">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center bg-gray-800 p-6 rounded-2xl shadow-lg">
          <div className="text-center md:text-left">
            ...
            <p className="text-gray-400 text-sm mt-2">Chance of rain: {weatherData?.rain?.["1h"] || 0}%</p>
            <h2 className="text-7xl font-extrabold mt-4">{Math.round(weatherData.main.temp)}°</h2>
            {currentDay && (
              <div className="mt-2 text-sm text-gray-400">
                High: {Math.round(currentDay.main.temp_max)}° / Low: {Math.round(currentDay.main.temp_min)}°
              </div>
            )}
          </div>
        ...
        <section className="mt-8">
          <h3 className="mb-4 text-gray-400 font-semibold text-sm">TODAY’S FORECAST</h3>
          <div className="flex justify-around overflow-x-auto gap-4 p-4 rounded-2xl bg-gray-800">
            {weatherData.hourly.map((h) => (
              <div key={h.dt} className="flex flex-col items-center min-w-[70px]">
                ...
                <p className="text-white font-semibold capitalize text-sm">{h.weather[0].main}</p>
      ...
      <aside className="w-full md:w-64 mt-6 md:mt-0 md:ml-8">
        ...
        <div className="bg-gray-800 rounded-2xl p-4 space-y-4">
          {weatherData.daily.map((d) => (
            <div key={d.dt} className="flex justify-between items-center p-3 rounded-lg bg-gray-700/50">
